...
import { Eye, EyeOff, DollarSign, TrendingUp, PiggyBank, Wallet, CreditCard, Receipt, BarChart3, Calculator, Coins, Banknote } from 'lucide-react';
...
const TechOrbitDisplay = memo(function TechOrbitDisplay({
  ...
  return (
    <section className='relative flex h-full w-full flex-col items-center justify-center overflow-hidden rounded-lg'>
      <span className='pointer-events-none whitespace-pre-wrap bg-gradient-to-b from-primary to-primary/50 bg-clip-text text-center text-7xl font-semibold leading-none text-transparent dark:from-primary dark:to-primary/50'>
        {text}
      </span>
  ...
  const validateForm = (event: FormEvent<HTMLFormElement>) => {
    ...
    fields.forEach((field) => {
      ...
      if (field.type === 'password' && value && value.length < 6) {
        currentErrors[field.label] = 'Senha deve ter pelo menos 6 caracteres';
      }
    });
  ...
  return (
    <section className='max-md:w-full flex flex-col gap-4 w-96 mx-auto'>
      ...
      <form onSubmit={handleSubmit}>
                  ...
                  {field.type === 'password' && (
                    ...
                    >
                      {visible ? (
                        ...
                        <EyeOff className='h-5 w-5 text-neutral-400 dark:text-neutral-600' />
                      )}
                    </button>
                  )}
                </section>
        ...
        >
          <button
            className='bg-gradient-to-br relative group/btn from-primary/90 dark:from-primary/90
            dark:to-primary/90 to-primary/90 block dark:bg-primary w-full text-white
            dark:text-white rounded-md h-10 font-medium shadow-[0px_1px_0px_0px_#ffffff40_inset,0px_-1px_0px_0px_#ffffff40_inset] 
              dark:shadow-[0px_1px_0px_0px_hsl(var(--primary))_inset,0px_-1px_0px_0px_hsl(var(--primary))_inset] outline-hidden hover:cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed'
            type='submit'
            disabled={loading}
          >
            {loading ? 'Carregando...' : submitButton} {!loading && '→'}
            <BottomGradient />
          </button>
        </BoxReveal>
...
const BottomGradient = () => {
  return (
    <>
      ...
      <span className='group-hover/btn:opacity-100 blur-sm block transition duration-500 opacity-0 absolute h-px w-1/2 mx-auto -bottom-px inset-x-10 bg-gradient-to-r from-transparent via-primary/70 to-transparent' />
    </>
  );
};
...
// ==================== Financial Icons Array ====================

export const financialIconsArray: IconConfig[] = [
  {
    component: () => (
...
// ==================== ModernAnimatedSignIn Component ====================

interface ModernAnimatedSignInProps {
  fieldPerRow?: number;
  onSubmit: (event: FormEvent<HTMLFormElement>) => void;
  goTo?: (event: React.MouseEvent<HTMLButtonElement>) => void;
  header: string;
...
const ModernAnimatedSignIn = memo(function ModernAnimatedSignIn({
  fieldPerRow = 1,
  onSubmit,
  goTo,
  header,
  ...
  errorField,
  loading = false,
}: ModernAnimatedSignInProps) {
  return (
    <div className="w-full max-w-md mx-auto bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl p-8">
      <AnimatedForm
        header={header}
        subHeader={subHeader}
        fields={fields}
        submitButton={submitButton}
        textVariantButton={textVariantButton}
      ...
      {onGoogleSignIn && (
        <div className="mt-6">
          ...
          <button
            ...
            className="mt-4 w-full flex justify-center items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 transition-colors"
          >
